...
つまり実測ではなく経験則なので、あくまで**暫定値として運用**しています。見直すときの観測点は「指摘の的中感が落ち始めるファイル数」「段階を分けても重複・的外れな指摘が増えていないか」あたりで、ここがズレてきたら数字を動かす前提です。
...
判断の構造としては「SaaS の追加課金 + 汎用的なレビュー観点」vs「手元の Cursor / Claude + 自社規約の強制力・カスタマイズ性」というトレードオフで、後者を取った形です。毎 push の完全自動レビューがチームの必須要件になった時点が、CodeRabbit 等の導入（併用）を再検討する条件だと考えています。
...
この仕組みが機能し続けているかどうかは、次の 3 つを観測点として見ていくつもりです。

- **投稿率**: AI レビューを回したのに GitHub に残っていない PR が出ていないか（出始めたら「Yes 1 回」の摩擦がまだ高い証拠）
- **検索ヒットの実感**: あとから `Cursor self-review` で検索してコメントに助けられた場面が実際にあるか
- **形骸化のサイン**: フッタ付きコメントが誰にも読まれず放置されていないか
