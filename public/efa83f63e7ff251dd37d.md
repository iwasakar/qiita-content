...
---
## 概要
すでに稼働しているDocker PJにSchemaSpyを追加して手軽にER図を無料で自動生成する方法を記載します。
...
## いつこれが効くか
ER図の自動生成が特に効くのは、**新メンバーのオンボーディング**、**テーブル数が多く手書きER図がメンテされなくなった既存DB**、**スキーマ変更レビューで関連テーブルをサッと確認したいとき**あたりです。
手書きのER図は設計意図を表現できる代わりにすぐ古くなる、SchemaSpyは常にDBの実態を映す代わりに設計意図は載らない、という使い分けになります。
「いまの実態を正確に知りたい」が目的なら、自動生成が向いています。
...
下記のように出ているか、SchemaSpyコンテナが停止すれば出力が完了。
```
...
## 生成して終わりにしない
ドキュメントは生成した瞬間から古び始めます。コマンド1発で再生成できるので、**マイグレーション後に叩き直す**運用に組み込んで初めて価値が出ます。
CIに組み込む選択肢もありますが、上記の通り170テーブルで10分程度かかるので、PRごとではなく夜間バッチなど頻度を選ぶのが現実的です。
