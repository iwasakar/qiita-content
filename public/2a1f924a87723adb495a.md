---
title: GASとCrowdLog APIで、チームの勤怠入力を自動チェックしてSlackに爆速通知する
tags:
  - GoogleAppsScript
  - Slack
  - Gemini
...
## 作る前の判断 — なぜ「GASで内製」だったのか

仕組みの解説に入る前に、コードを書く前に考えたことを正直に書いておきます。選択肢は3つありました。

| 選択肢 | 得られるもの | 引き換えに捨てるもの |
| --- | --- | --- |
| ① 手動リマインドを続ける | 追加コストゼロ・今すぐできる | 確認する人の時間と「催促する側のストレス」が永遠に残る |
| ② つよつよ高級勤怠管理連携ツールを導入する | メンテ不要・機能も豊富 | 追加費用と導入の社内調整。「入力チェックと通知」だけが欲しい今は明らかにオーバースペック |
| ③ GASで内製する | 追加費用ゼロ・既存アカウントだけで完結 | メンテ責任は自分。属人化リスクとGASの実行制限も背負う |

判断軸はシンプルで、**「追加費用ゼロ」「既存のGoogle・Slackアカウントで完結する」「欲しい機能は入力チェックと通知だけ」**という制約・要件でした。この条件なら、稟議も予算も不要で今日から始められる③一択です。
逆に言えば、チェック対象が全社規模に広がったり、勤怠の締め処理まで自動化したくなったりした時点で、②への乗り換えを検討する条件が揃う、と線引きしています。

### 内製で「捨てたもの」と、その緩和策

③を選んだことで背負ったものも、隠さず書いておきます。

- **属人化リスク**: 作った本人しか直せない状態が一番危険です。なのでコードはチームで共有し、この記事のように仕組み自体を文書化しています。
- **GASの実行制限**: GASには1日あたりの合計実行時間やUrlFetchの呼び出し回数に上限があります。今回の規模（チーム単位・1日2回）なら余裕ですが、対象を増やすならこの制限を意識した設計が必要です。
- **メンテナンス責任**: メンバーの増減やチーム再編のたびに修正が発生します。だからこそ設定を`TEAMS_CONFIG`に一点集約し、「コード全体を読まなくても編集箇所が分かる」状態にしています（後述）。
...
チームの追加やメンバーの変更、通知先Slackチャンネルの変更が、この`TEAMS_CONFIG`オブジェクトを編集するだけで完結するようにし、メンテナンス性を向上させています。冒頭で挙げた「メンテ責任は自分」という内製の弱点を、せめて「編集箇所が1か所に決まっている」状態にして緩和する狙いです。
今後ここにチームごとの閾値調整や、時間変更なども含めることも可能です。
GASを最大限活かすなら、スプレッドシートの入力値と連動させるのが次の一手です。設定がシートに出れば、非エンジニアのリーダーでもメンバー更新ができるようになり、属人化リスクをさらに下げられます。
...
ただ、一段掘っておくと、この仕組みは入力忘れを「検知」してくれますが、入力忘れが「起きる」こと自体はなくしてくれません。
そもそもなぜ忘れるのかと言えば、工数入力という行為が日々の業務フローから切り離されているからです。タスクを閉じる場所と工数を入力する場所が別ツールなら、忘れるのはむしろ自然です。

なので次の一手は、「通知で思い出させる」から「業務の流れの中で入力が終わる」への移行が観点になります。タスク完了の動線に入力を組み込む、Slackの通知から最短で入力画面に飛べる導線にする、といった方向です。
通知はそこに辿り着くまでの応急処置──ただし、追加費用ゼロで今日から効く応急処置です。まずここから始める価値は十分にあります。
